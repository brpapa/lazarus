...
function LabelWithIcon(props: LabelWithIcon) {
  ...
  const { fontStyles, onPressItem, left, style, label, icon, ...otherProps } = props
...
function LabelOnly(props: LabelOnlyProps) {
  ...
  return (
    <TouchableOpacity onPress={onPressItem} {...otherProps} style={[!left && styles.right, style]}>
...
export function HeaderItem(props: HeaderItemProps) {
  ...
  let { label = '', icon, loading = false, disabled = false, style, ...otherProps } = props
  ...
  if (icon) {
    return <IconOnly icon={icon} disabled={disabled} style={itemStyle} {...otherProps} />
  }
  ...
  return <LabelOnly label={label} disabled={disabled} style={itemStyle} {...otherProps} />
}
