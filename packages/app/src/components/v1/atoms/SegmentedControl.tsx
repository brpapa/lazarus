...
export function SegmentedControl<T>(props: Props<T>) {
  ...
  const IOS_SEGMENT_WIDTH = (width - (spacing.xxl * 2 + spacing.s)) / values.length
  ...
  return (
    <View style={styles.container}>
      ...
      {values.map((value, index) => (
        ...
        >
          <Text style={[styles.text, selectedIndex === index && styles.textChosen]}>
...
const useStyles = makeUseStyles(({ colors, fontVariants, shadow, spacing }) =>
  __IOS__
    ? {
        container: {
